...
export class LogsDb {
  ...
  public static saveLog = async (branchId: string, severity: LogsSevierity, user: string, payload: any, message: string) => {
    const id = v4();
  ...
  public static updateError = async (logId: string, message: string) => {
    await DB.getInstance().update<Log>(this.TABLE_NAME, 'id', logId, { message }, ['id']);
  };
}
